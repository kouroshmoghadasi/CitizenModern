# -*- coding: utf-8 -*-
"""
Build static/exam2_questions.json from History of Canada1.docx.
Uses chapter3_questions.json (and others) for FA/FR when option count matches and similarity is high.
"""
import difflib
import json
import os
...
OUT = os.path.join(ROOT, "static", "exam2_questions.json")
DOCX = r"e:\OK Projects\Nemoneh Question Citizen CA\History of Canada1.docx"

# Verified against Discover Canada / standard citizenship study material (54 questions)
CORRECT = [
    2, 0, 0, 3, 1, 1, 2, 2, 3, 0, 2, 3, 0, 2, 3, 2, 2, 3, 0, 0, 3, 1, 2, 3, 3, 0, 1, 2, 3, 0, 3, 1, 3, 0, 3, 2, 2, 0, 0, 1, 1, 3, 2, 1, 2, 0, 1, 0, 1, 3, 2, 2, 0, 0,
]
...
def _clean_text(s: str) -> str:
    ...
    s = s.replace("Mtis", "Métis").replace("marathon of hope", "Marathon of Hope")
...
def _build_options_lookup(bank: list):
    """Map normalized option-tuple -> (options_fa, options_fr) when lengths match."""
    fa_map = {}
    fr_map = {}
    for item in bank:
...
def _best_bank_match(q_en: str, n_opts: int, bank: list, min_ratio: float = 0.72):
    nq = _norm(q_en)
    best = None
    best_r = 0.0
    for item in bank:
...
def main():
    qs = _parse_docx(DOCX)
    assert len(qs) == 54, len(qs)
    assert len(CORRECT) == 54, len(CORRECT)
    ...
    for i, ((q_en, opts_en), cor) in enumerate(zip(qs, CORRECT)):
        ...
        item, ratio = _best_bank_match(q_en, len(opts_en), bank, min_ratio=0.68)
        q_fa = q_fr = None
    ...
    # گزینه‌هایی که در بانک کلید tuple نداشتند (دستی)
    _manual_opts_fa = {
        0: [
            "نیوبرانزویک، نوااسکوشیا، آلبرتا، مانیتوبا",
            "نیوبرانزویک، نیوفاندلند، ایالت کانادا (انتاریو و کبک)",
            "نیوبرانزویک، نوااسکوشیا، ایالت کانادا (انتاریو و کبک)",
            "مانیتوبا، نوااسکوشیا، ایالت کانادا (انتاریو و کبک)",
        ],
    }
    for idx, fals in _manual_opts_fa.items():
        if idx < len(out) and len(fals) == len(out[idx]["options_en"]):
            out[idx]["options_fa"] = fals
